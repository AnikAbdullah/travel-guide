...
class AuthController
{
    ...
    public function register(): void
    {
        ...
        elseif (strlen($old['name']) > 100)   $errors['name']  = 'Name must be 100 characters or fewer.';

        if ($old['email'] === '')             $errors['email'] = 'Email is required.';
        elseif (!validate_email($old['email'])) $errors['email'] = 'Enter a valid email address.';

        if ($password === '')                 $errors['password'] = 'Password is required.';
        elseif (!validate_password($password)) $errors['password'] = 'Password must be at least 8 characters.';
        ...
        if (!validate_role($old['role']))     $errors['role'] = 'Select a valid role.';
        ...
        flash('success', 'Registration successful. Your account is pending admin approval.');
        redirect('/login');
    }
    ...
    public function login(): void
    {
        ...
        if ($remember) set_remember_me($this->conn, (int) $user['id']);
    ...
    public function logout(): void
    {
        ...
        if ($userId !== null) clear_remember_me($this->conn, (int) $userId);
